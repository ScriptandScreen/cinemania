export function calcVote(vote) {
  const stars = {
    fill: `<svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
...
`,
    hybrid: `<svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
...
`,
  ...
  const hasHalfStar = vote % 2 >= 0.5;
  const emptyStars = count - fullStars - (hasHalfStar ? 1 : 0);
  let likes = stars.fill.repeat(fullStars); 
  if (hasHalfStar) likes += stars.hybrid; 
  likes += stars.empty.repeat(emptyStars); 
  return likes;
}
