      ...
      descriptionContainer.innerHTML = `
        ...
        <div class="desc-button-container">
          ...
          <div class="hero-movie-buttons">
            <button id="trailer" data-trailer-id="${movie.id}" class="orange-button btn-hero trailer">Watch trailer</button>
            <button id="details" data-details-id="${movie.id}" class="white-button btn-hero details">More details</button>
          </div>
        </div>
      `;
  ...
  const getMovies = async () => {
    ...
    movies.forEach(async movie => {
      if (count <= 3) {
        ...
        count++;
        movieHtml += `
            <div class="catalog-movie-card" data-movie-id="${movie.id}"><img
